...
def join_button(request):
    ...
    start_r_datetime = datetime(2025, 3, 9, 18)  # 합격자 조회 시작 날짜와 시간(마지막이 시간. 오후 9시부터 시작 -> 21로 입력.)(여기는 1차 때와 최종 합격 때 바꿔야함)
    end_r_datetime = datetime(2025, 3, 12)  # 합격자 조회 끝나는 날짜와 시간(여기는 1차 때와 최종 합격 때 바꿔야함)
    isDate = 0
